...
function showQRaddress(e) {
	$(e).fadeOut(function() {
		new QRCode(document.getElementById("qr-address"), {
			text: $('#iota-address').val(),
			width: 160,
			height: 160
		});
		$('#qr-address').css('display','flex');
	});
}

function openPriceChart() {
	$('#chart-frame').on('load',function() {
		$('#chart-loader').removeClass('is-active');
	});
	$('#chart-frame').attr('src','iframe.chart.php');
	$('#price-chart').modal({
		backdrop: 'static',
		keyboard: false
	});
...
function generatePDF(seed) {
	...
	setTimeout(function() {
		...
		doc.addImage(logo,'PNG', 78, 22, 60, 60);
		doc.addImage(QRcode,'PNG', 167, 4, 38, 38);
				...
				if (isSeedValid(seed)) {
					setTimeout(function() {
						...
						if (command == 'delete-account') {
							$.post('ajax.delete-account.php',{confirm:true},function(response) {
								if (response == 1) {
									$('.loader').removeClass('is-active');
									showBSModal({
										title: 'Account deleted!',
										body: '<div class="text-center"><b>Account has been successfully deleted!</b></div>',
										size: "small",
										actions: [{
											label: 'Close',
											...
											onClick: function(e) {
												...
												window.location.href = window.location.protocol+'//'+window.location.hostname+'/logout.php';
											}
										}]
									});
								}
									...
									$('#confirm-pin').find('.modal-footer').html(button);
									alert('ERROR: Unable to delete account, try again or contact support!');
								}
							});
						}
						// Generating address
